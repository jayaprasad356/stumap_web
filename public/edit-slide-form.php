...
if (isset($_POST['btnEdit'])) {
		...
		if (!empty($name)) {
			if ($_FILES['image']['size'] != 0 && $_FILES['image']['error'] == 0 && !empty($_FILES['image'])) {
				...
				$upload_image = 'upload/slides/' . $filename;
				$sql = "UPDATE slides SET `image`='" . $upload_image . "' WHERE `id`=" . $ID;
				$db->sql($sql);
			}
			
             $sql_query = "UPDATE slides SET name='$name',status='$status' WHERE id =  $ID";
			 $db->sql($sql_query);
			...
			if ($update_result == 1) {
				...
				$error['update_slide'] = " <span class='label label-danger'>Failed update slide</span>";
			}
		}
	} 
...
<section class="content-header">
	<h1>
		Edit Slide<small><a href='slides.php'><i class='fa fa-angle-double-left'></i>&nbsp;&nbsp;&nbsp;Back to Slides</a></small></h1>
